...
coll_names = [collection for collection in db_aws.list_collection_names()]
for col_name in coll_names:
    print("local ", col_name, " 삭제")
    db_local[col_name].drop()

    docs = list(db_aws[col_name].find({}, {'_id': False}))
    db_local[col_name].insert_many(docs)
    print("aws -> local: collection", col_name)